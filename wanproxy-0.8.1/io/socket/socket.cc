...
void Socket::socket_alive(int sockfd,int idle)
{
	int opt=1;
	int keepIdle = idle; /* after how many seconds start to detect when no data transfer */
	int keepInterval = 30; /* 60 seconds */
	int keepCount = 5; /* detect 5 times */
...
Socket::accept_callback(Event e)
{
	...
	//char addr[32];
	int s = ::accept(fd_, (struct sockaddr *)&addr, &sin_size);
	...
	socket_alive(s, ALIVE_TIME);
	Socket *child = new Socket(s, domain_, socktype_, protocol_);
//	printf("Accept ok\n");
	//snprintf(addr, 32, "[%s]:%u", inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
	//child ->dest = std::string(addr);
	//
	child->src_addr = addr;
